...
uint8_t number;//задаваемый с помощью перемычек номер устройства
...
//======программа дешифрации команды из рекламного имени=======================
static void decryption_name(uint8_t *name, uint8_t name_len)
{
    //получение имени из буфера с указанным адресом
    char full_name[name_len];
    strcpy(full_name, (char *)name);//копирование строки
    ...
    //если номер в имени и номер устройства совпадают или номер нулевой (широковещательная рассылка)
    if(((full_name[13]==number_ch[0])&&(full_name[14]==number_ch[1]))
        ||((full_name[13]== '0')&&(full_name[14]== '0')))
    {
        uint8_t level;
        int out = (full_name[18] -'0') + (full_name[17] - '0') * 10 
        + (full_name[16] - '0') * 100;//преобразование символьного числа в код
        //если код команды более 255
        if(out > 255)
        {
            ESP_LOGI(GATTC_TAG, "Команда включить - %d", (out - 256));
            level=1;
        } else {
            ESP_LOGI(GATTC_TAG, "Команда выключить - %d", (out - 256));
            level=0;
        }
            out = out - 256;
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT: {
        ...
        uint32_t duration = 4294836225;//сканирование 136 лет до сброса
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            //=========вставка моей программы дешифрации команды из рекламного имени=====
            if(adv_name != NULL){
                //если начало полученного имени в рекламе совподает с MySmartHouse
               if(!strncmp((char *)adv_name,"MySmartHouse",12))
               {
                   //вызов программы дешифрации
                decryption_name(adv_name, adv_name_len);   
               } 
            }
...
static void installing_gpio()
{
    ...
    gpio_reset_pin(out5);
    gpio_set_direction(out5,GPIO_MODE_OUTPUT);
    gpio_set_level(out5, 0);
    gpio_reset_pin(out6);
    gpio_set_direction(out6,GPIO_MODE_OUTPUT);
...
{
     installing_gpio();
    //==== Initialize NVS - используется для хранения данных калибровки PHY======= 
    ...
    if (ret) {
        ...
        ESP_LOGE(GATTC_TAG, "Bluetooth controller installing\n");
    //==================================================================================
    ...
    if (ret) {
        ...
        ESP_LOGE(GATTC_TAG, "Bluedroid installing\n");
    //===================================================================================
    ...
    if (ret){
        ...
        ESP_LOGE(GATTC_TAG, "ble_gap_register_callback installing\n");
    ...
    if(ret){
        ...
        return;
         ...
         ESP_LOGE(GATTC_TAG, "ble_gattc_register_callback installing\n");
